...
#date: "2024-05-23" # Or use `r Sys.Date()` if rendering via R/Quarto
format:
  html:
    ...
    css: styles.css       # Optional: link a custom CSS file if you have one
jupyter: python3
editor: visual
---
...
avec les contraintes **$a_1 < 0$** et **$a_2 < 0$**, assurant une décroissance exponentielle.
...
Comme $a_1 < 0$, $a_2 < 0$, et $e^{a_2 x} > 0$, nous avons toujours $\tau'(x) < 0$, ce qui garantit une **décroissance monotone** du taux effectif (progressivité fiscale).
...
Comme indiqué dans votre analyse précédente, cette équation admet **deux solutions** mathématiques (branches $k=0$ et $k=-1$). Nous calculons les deux et identifions celle qui a un sens économique (positive et dans la plage de revenus).
...
df_lambert["Seuil Retenu (k€)"] = df_lambert[["Solution k=0", "Solution k=-1"]].max(axis=1) # On prend la solution positive significative
display(df_lambert)
...
2. **Point de Bascule (~62k€) :** L'analyse mathématique (Lambert W) identifie un seuil autour de **62 000 € de brut annuel**.
...
**Implication pratique :** C'est autour de ce pivot de 62k€ que les stratégies d'optimisation (épargne retraite, défiscalisation) deviennent statistiquement les plus pertinentes pour contrer la décélération du gain net marginal.
